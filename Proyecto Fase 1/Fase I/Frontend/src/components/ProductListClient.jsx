...
const ProductListClient = () => {
  ...
  const fetchCategories = () => {
    axios
      .get(`http://saf1.${apiUrl}.nip.io/gom2/api/products-category`)
      .then((response) => {
  ...
  const fetchExchangeRates = () => {
    ...
    const promises = Object.entries(currencies).map(([key, id]) =>
      axios
        .get(`http://saf1.${apiUrl}.nip.io/gom5/convertir?id=${id}&cantidad=1`)
  ...
  const handleAddToCart = (productId) => {
    ...
    axios
      .post(
        `http://saf1.${apiUrl}.nip.io/pythonm3/auth/cart/add`,

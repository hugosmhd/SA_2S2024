...
const ProductList = () => {
  ...
  const fetchProducts = () => {
    ...
    // setProducts(pro);
    axios
      .get('http://127.0.0.1:3000/api/products-provider/1')
      .then((response) => {
  ...
  const onUpdateProduct = (newProduct, respuesta) => {
    setProducts(products.map(product => (product.ProductID === newProduct.ProductID ? newProduct : product)));
    // if (respuesta.message) {
    //   notifySuccess(respuesta.message);
    // } else if (respuesta.error) {
    //   notifyError(respuesta.error);
  ...
  const onDeleteProduct = (bookId, respuesta) => {
    setProducts(products.filter(product => product.ProductID !== bookId));

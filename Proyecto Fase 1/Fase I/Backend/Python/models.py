...
class User(db.Model):
    ...
    tipo_usuario = db.Column(db.Enum("Proveedor", "Cliente"), nullable=False)
...
class Producto(db.Model):
    ...
    nombre_producto = db.Column(db.String(255), nullable=False)  # Cambiado para coincidir con la base de datos
    precio = db.Column(db.Numeric(10, 2), nullable=False)
    stock = db.Column(db.Integer, nullable=False)
    id_categoria = db.Column(db.Integer, db.ForeignKey("Categorias.id_categoria"), nullable=False)
    categoria = db.relationship("Categoria", backref="productos")  # Relación con Categorias
